        ...
        for (int y = 0; y < size; y++)
        {
            for (int x = 0; x < size; x++)
            {
                ...
                if (cell.isWater)
                    ...
                    colorMap[y * size + x] = new Color(155 / 255f, 191 / 255f, 115 / 255f);
            }
        }
    ...
    private void GenerateTrees(Cell[,] grid)
    {
        ...
        for (int y = 0; y < size; y++)
        {
            for (int x = 0; x < size; x++)
            {
                ...
                if (!cell.isWater)
                {
                    float v = Random.Range(0, treeDensity);
                    if (noiseMap[x, y] < v)
                    {
                        GameObject prefab = treePrefabs[Random.Range(0, treePrefabs.Length)];
                        GameObject tree = Instantiate(prefab, transform);
                        tree.transform.position = new Vector3(x + Random.Range(-.2f, .2f), 0, y + Random.Range(-.2f, .2f));
                        tree.transform.rotation = Quaternion.Euler(0, Random.Range(0, 360f), 0);
                        float scaling = Random.Range(-.1f, .1f);
                        tree.transform.localScale += new Vector3(scaling, scaling, scaling);
                        _trees.Add (tree);
                    }
                }
            }
